...
SECTIONS
{
  ...
  far1 (BANKSIZE=65536) : { *(SORT_BY_SIZE(*text* *_rodata.far.*))} >ram
  far2 (BANKSIZE=65536) : { *(SORT_BY_SIZE(*_data.far.* *.far.*))} >ram
  huge : { *(*data.*) *(*_bss.*) } >ram

  ___heap = ADDR(huge) + SIZEOF(huge);
  ___heapend = RAMSIZE-STACKLEN;
